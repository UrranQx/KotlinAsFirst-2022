...
class PhoneBook {
    ...
    fun addHuman(name: String): Boolean {
        when (name) {
            in book.keys -> return false
            else -> book[name] = mutableSetOf()
        }
        return true
    }
    ...
    fun addPhone(name: String, phone: String): Boolean {
        when {
            name !in book.keys || book[name]?.contains(phone) == true -> return false
            book.values.any { phone in it } -> return false
            else -> book[name]?.add(flattenPhoneNumber(phone))
        }
        return true
    ...
    fun removePhone(name: String, phone: String): Boolean {
        when {
            name !in book.keys || !book[name]!!.contains(phone) -> return false
    ...
    fun humanByPhone(phone: String): String? {
        return when {
            book.values.all { phone !in it } -> null
            else -> book.filterValues { phone in it }.keys.first().toString()
        }
    }
    ...
    override fun equals(other: Any?): Boolean = other is PhoneBook && book == other.book/*{
        ...
        if (book.keys.size != other.book.size) return false
        for ((name, phones) in book) if (phones != other.book[name]) return false
        return true
    }*/
